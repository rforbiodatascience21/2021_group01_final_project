...
Corr_plot <- ggpairs(Data_aug,
  ...
  labs(title = 
         "Correlation plots of predictive variables stratified on diagnosis") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_viridis(discrete = TRUE)

ggsave(Corr_plot, filename="/cloud/project/results/04_Corr_plot.png", 
...
plt3 <- Data_aug %>% 
  mutate(Chest_pain_type = case_when(!is.na(Chest_pain_type) ~ "Not NA")) %>%
...
plt4 <- Data_aug %>% 
  mutate(Resting_blood_pressure = 
           case_when(!is.na(Resting_blood_pressure) ~ "Not NA")) %>%
...
plt5 <- Data_aug %>% 
  mutate(Serum_cholestoral = 
           case_when(!is.na(Serum_cholestoral) ~ "Not NA")) %>%
...
plt6 <- Data_aug %>% 
  mutate(Fasting_blood_sugar = 
           case_when(!is.na(Fasting_blood_sugar) ~ "Not NA")) %>%
...
plt7 <- Data_aug %>% 
  mutate(Resting_electrocardiographic = 
           case_when(!is.na(Resting_electrocardiographic) ~ "Not NA")) %>%
...
plt8 <- Data_aug %>% 
  mutate(Maximum_heart_rate_achieved = 
           case_when(!is.na(Maximum_heart_rate_achieved) ~ "Not NA")) %>%
...
plt9 <- Data_aug %>% 
  mutate(Exercise_induced_angina = 
           case_when(!is.na(Exercise_induced_angina) ~ "Not NA")) %>%
...
plt10 <- Data_aug %>% 
  mutate(ST_depression_induced_by_exercise = 
           case_when(!is.na(ST_depression_induced_by_exercise) ~ "Not NA")) %>%
...
plt11 <- Data_aug %>% 
  mutate(The_slope_of_the_peak_exercise_ST_segment = 
           case_when(
             !is.na(The_slope_of_the_peak_exercise_ST_segment) ~ "Not NA")) %>%
...
plt12 <- Data_aug %>% 
  mutate(Number_of_major_vessels_colored_by_flourosopy = 
           case_when(
             !is.na(Number_of_major_vessels_colored_by_flourosopy) 
             ~ "Not NA"))%>%
...
plt14 <- Data_aug %>% 
  mutate(Diagnosis_of_disease = 
           case_when(!is.na(Diagnosis_of_disease) ~ "Not NA")) %>%
...
ggsave(NA_Bar, filename="/cloud/project/results/04_NA_Bar.png", 
       width = 16, height = 9, dpi = 72)


# Summary density plots---------------------------------------------
pltchol = Data_aug %>% ggplot(aes(Serum_cholestoral)) + 
  geom_density(aes(color = Diagnosis_of_disease, 
...
ggsave(Summary_density, 
       filename="/cloud/project/results/04_Summary_density_plots.png", 
       width = 16, height = 9, dpi = 72)

#Summary bar plots ---------------------------------------------
plot_sex = Data_aug  %>% 
  group_by(Diagnosis_of_disease) %>% 
  count(Sex_cat) %>%
...
plot_electro = Data_aug  %>% 
  group_by(Diagnosis_of_disease) %>% 
  count(Resting_electrocardiographic_cat) %>%
...
plot_exercise = Data_aug  %>% 
  group_by(Diagnosis_of_disease) %>% 
  count(Exercise_induced_angina_cat) %>%
...
Summary_table <- Data_aug %>% 
  ...
  mutate("mean w. sd" = paste0(round(mean, 2), 
                               " (", intToUtf8("177"), 
                               round(sd, 2), ")")) %>% 
