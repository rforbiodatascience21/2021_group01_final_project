...
Data_aug <- Data %>%
  mutate(Location_num = factor(case_when(Location == "Cleveland" ~ 0, 
         ...
         Diagnosis_of_disease = factor(case_when(diagnosis_of_heart_disease == 0 ~ "Not present", 
         ...
         Resting_electrocardiographic_cat = factor(case_when(Resting_electrocardiographic == 0 ~ "Normal", 
         ...
         Age_class = factor(case_when(Age < 20 ~ ">20",
                   20 <= Age & Age < 30 ~ "20-30",
                   30 <= Age & Age < 40 ~ "30-40",
                   40 <= Age & Age < 50 ~ "40-50",
                   50 <= Age & Age < 60 ~ "50-60",
